...
func New(config *LoggingConfig) *zap.Logger {
	...
	jsonOutCore := zapcore.NewCore(zapcore.NewConsoleEncoder(productionCfg), stdout, level)
	...
	return zap.New(samplingCore, zap.AddCaller(), zap.AddStacktrace(zap.ErrorLevel))
}
